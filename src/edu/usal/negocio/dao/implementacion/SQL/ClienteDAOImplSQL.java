...
public class ClienteDAOImplSQL implements ClienteDAO {
	...
	private Direccion leerDireccion(int dni)throws SQLException { // cliente-pais-provincia-ciudad-codpostal-calle-altura
		...
		ResultSet rs = prep.executeQuery();
		rs.next();
		return new Direccion(rs.getString(6), rs.getString(7), rs.getString(4), new Provincia(rs.getInt(3),DatosEstaticos.getProvincias().get(rs.getInt(3))), new Pais (rs.getInt(2),DatosEstaticos.getPaises().get(rs.getInt(2))), rs.getString(5));
	}
	private Pasaporte leerPasaporte(int dni)throws SQLException {	// ID_Cliente-NumPasaporte-AutoridadEmision-Pais-FechaEmision-FechaVenc
		...
		ResultSet rs = prep.executeQuery();
		rs.next();
		return new Pasaporte(rs.getString(2), new Pais(rs.getInt(4),DatosEstaticos.getPaises().get(rs.getInt(4))), rs.getString(3), convertFromSQLDateToJAVADate(rs.getDate(5)), convertFromSQLDateToJAVADate(rs.getDate(6)));
	}
